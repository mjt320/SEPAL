...
def s_to_pkp(t, s, c_p_aif, r_0_tissue, r_0_blood, rlxy_pars, acq_pars, hct, irf_model, c_to_r_model, r_to_s_model, water_model='fxl', delta_t_interp=1.0, fit_opts=None):
    ...
    # max_t = np.max(t)
    # n_interp = np.round(max_t/delta_t_interp + 0.5).astype(int)
    # delta_t_interp_actual = max_t/(n_interp-0.5)
    # t_interp = np.linspace(0.5*delta_t_interp_actual, max_t, num=n_interp)
    # calculate idx of interpolated points corresponding to sampled points
    # c_p_aif_interp = interp1d(t_interp, c_p_aif, kind='quadratic', bounds_error=False, fill_value=c_p_aif[0])    
